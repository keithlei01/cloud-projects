...
if (require.main === module) {
  ...
  async function authenticationError(): Promise<string> {
  ...
  try {
    const result = await retryManager.executeWithRetry(operation1);
  ...
  try {
    const result = await retryManager.executeWithRetry(operation2);
    console.log(`Result: ${result}`);
  } catch (error) {
  ...
  try {
    const result = await retryManager.executeWithRetry(operation3);
    console.log(`Result: ${result}`);
  } catch (error) {
    console.log(`Expected failure: ${error}`);
  }
  ...
  // Print metrics
  console.log("\nMetrics:");
  for (const operationType of ["api_call", "auth", "always_fails"]) {
    const metrics = retryManager.getMetrics(operationType);
    console.log(`${operationType}: ${JSON.stringify(metrics, null, 2)}`);
  }
  ...
  for (const op of failedOps) {
    console.log(`Failed operation: ${op.id} - ${op.error_message}`);
  }
}
